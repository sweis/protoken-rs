...
impl Serialize for KeyIdentifier {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        let (type_name, bytes): (&str, &[u8]) = match self {
            KeyIdentifier::KeyHash(hash) => ("key_hash", hash),
            KeyIdentifier::PublicKey(pk) => ("public_key", pk),
        };
...
impl Claims {
    ...
    pub fn validate(&self) -> Result<(), ProtokenError> {
        ...
        for scope in &self.scopes {
            // Empty scopes are rejected because canonical encoding omits
            // empty fields — an empty entry would silently vanish on the wire.
            if scope.is_empty() {
                return Err(ProtokenError::MalformedEncoding(
                    "scope entry must not be empty".into(),
                ));
            }
...
/// tokens, but the envelope itself is payload-agnostic.
#[derive(Debug, Clone, PartialEq, Eq)]
